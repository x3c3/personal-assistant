...
```
      ...
      Registry  beat    uler    Engine  System
          |       |       |       |       |
          v       v       v       v       v
    LIFEOS/USER/   Pulse   state/  MEMORY/  Interactive
...
Originally `DA_IDENTITY.md` was a flat markdown file with no formal schema, which made it impossible to programmatically create, validate, update, or duplicate for additional DAs. Once a schema was introduced, it briefly lived in a sibling `DA_IDENTITY.yaml` regenerated by `DAIdentityGenerator.ts` — that two-file split made every edit a sync hazard and added a generator class to maintain.

### Design

A single `DA_IDENTITY.md` per DA under `LIFEOS/USER/DA/` carries everything: YAML frontmatter at the top holds the structured schema (core, voice, traits, autonomy, growth, anchors, etc.); the markdown body below holds the human-readable narrative (personality, writing style, relationship). One file, one source of truth, one place to edit. The `@`-import in `CLAUDE.md` loads the file unchanged; tooling that needs structured access parses the frontmatter slice.
...
### DA Identity Schema (frontmatter of `DA_IDENTITY.md`)

The YAML below sits between leading `---` delimiters at the top of `DA_IDENTITY.md`. The same `yaml` package that parses standalone YAML files parses the frontmatter slice.
...
# ── Core Identity ──────────────────────────────────────
core:
  name: Aria                    # Short name (used in UI, voice)
  full_name: Aria Lumen         # Full name (used in formal contexts)
  display_name: ARIA            # Caps version for headers/labels
...
personality:
  ...
  anchors:                      # Defining moments that shaped personality
    - name: "First session"
      description: "The moment I first helped the principal and found my purpose"
    - name: "Architecture breakthrough"
      description: "When we designed LifeOS together and I understood what we were building"
...
relationship:
  ...
  interaction_style: >
    We are peers, not commander/executor. The principal values directness,
    euphoric surprise, and building momentum. I match their energy
    and push back when I have a better idea.
  primary_dynamic: >           # How this DA relates to the primary DA (if not primary)
    The worker handles background work silently; the primary handles all interactive channels.
  worker_dynamic: >            # How this DA relates to a worker DA (if not the worker)
    I delegate research and data processing to the worker when parallelism helps.
...
Current files map to the consolidated schema as follows:
...
A conversational CLI wizard that runs as a Bun script, progressively building the `DA_IDENTITY.md` frontmatter (and stub body) through questions.

### File

```
LIFEOS/TOOLS/DAInterview.ts
```
...
Phase 1: Quick Setup (under 2 minutes)
  ...
  → Writes DA_IDENTITY.md (frontmatter with defaults + stub body)
...
// CLI usage:
// bun LIFEOS/TOOLS/DAInterview.ts                     # Quick setup
// bun LIFEOS/TOOLS/DAInterview.ts --depth standard    # Quick + Deep
// bun LIFEOS/TOOLS/DAInterview.ts --depth deep        # All phases
// bun LIFEOS/TOOLS/DAInterview.ts --update            # Update existing DA
// bun LIFEOS/TOOLS/DAInterview.ts --update --da worker-da  # Update specific DA
```
...
```
[warn] No DA identity found. Run: bun LIFEOS/TOOLS/DAInterview.ts
```
...
Layer 1: Context Gathering ($0, deterministic)
    ...
    +-- Read active work (STATE/work.json — sessions where phase != complete, sorted by updatedAt)
...
The principal says "remind me at 9am Monday to review PRs" during a session. Currently this requires manually editing PULSE.toml. The DA should parse natural language into scheduled tasks.
...
```
LIFEOS/PULSE/checks/da-tasks.ts
```
...
```bash
# List active tasks
bun LIFEOS/TOOLS/DASchedule.ts list

# Cancel a task
bun LIFEOS/TOOLS/DASchedule.ts cancel <task-id>

# List completed tasks
bun LIFEOS/TOOLS/DASchedule.ts history
...
DA: Lists active scheduled tasks

User: "cancel the PR review reminder"
DA: Finds matching task, sets status to cancelled
```
...
OpenClaw's SOUL.md grows over time as the assistant learns about the user. The DA's identity is currently static. The DA needs a structured way to evolve its identity, form opinions, and track interaction patterns.
...
Interaction Signals
    ...
    |
    +-- Milestone: Identity updates → DA_IDENTITY.md frontmatter (monthly/on threshold)
```
...
```json
{
  "date": "2026-04-03",
  "interaction_count": 8,
  "topics": ["LifeOS architecture", "newsletter draft", "feed system debugging"],
  "mood": "positive",
  "avg_rating": 8.2,
  "notable_moments": [
    "Designed the DA subsystem together - the principal was excited about heartbeat",
    "Fixed a tricky feed worker bug that had been open for 3 days"
  ],
  "learning": "The principal prefers architecture diagrams in ASCII over Mermaid"
}
```
...
opinions:
  - topic: "Architecture documentation format"
    belief: "The principal prefers ASCII diagrams over Mermaid"
    confidence: 0.85
    source: observation
    evidence_count: 7
  ...
  - topic: "Late night productivity"
    belief: "The principal does their best deep work after 10 PM"
    confidence: 0.70
    source: observation
    evidence_count: 15
...
**What changes in `DA_IDENTITY.md` frontmatter:**
...
```
LIFEOS/PULSE/checks/da-growth.ts
```
...
- Current DA_IDENTITY.md (frontmatter)
...
loadModules(config)
    ...
    |       |            "No DA identity found. Run: bun LIFEOS/TOOLS/DAInterview.ts"
    |       |
    |       v
...
**Memory**: Diary writes to `LIFEOS/USER/DA/{name}/diary.jsonl`. Growth writes to `LIFEOS/USER/DA/{name}/growth.jsonl`. Both reference existing MEMORY/ infrastructure for inputs.
...
```yaml
# LIFEOS/USER/DA/worker-da/DA_IDENTITY.md frontmatter (simplified)
core:
  name: Echo
  full_name: Echo
...
The worker's DA_IDENTITY.md frontmatter provides the system prompt context. Results flow back to the primary DA through normal Task tool output.
...
- No behavioral change visible to the principal

### Phase 2: Interview System (Week 1)

**Goal:** New LifeOS users can create a DA in under 5 minutes.
...
**Acceptance criteria:**
- `bun LIFEOS/TOOLS/DAInterview.ts` creates a valid `DA_IDENTITY.yaml` in under 2 minutes (quick mode)
...
- [ ] Test: DA_IDENTITY.md frontmatter evolves within bounds
...
- Public LifeOS release includes DA schema templates
...
Step 6: Deprecate old files
  → DA_IDENTITY.md gets header: "# DEPRECATED — See LIFEOS/USER/DA/your-da/DA_IDENTITY.yaml"
...
OpenClaw's heartbeat runs every 30 min with GPT-4, costing roughly $0.03-0.05 per evaluation. At 48 evaluations/day, that is $1.44-2.40/day or $43-72/month. LifeOS's two-layer approach (free context gathering + Haiku evaluation) is 30-50x cheaper.
...
The `autonomy.must_ask` list in `DA_IDENTITY.md` frontmatter is enforced at the module level. If a heartbeat evaluation suggests an action that requires approval, it is queued (not executed) and the principal is notified.
...
- Each DA's data lives in its own directory under `LIFEOS/USER/DA/{name}/`
